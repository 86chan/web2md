...
- **自動分割**: LLMのコンテキスト制限を考慮し、指定したファイルサイズまたは単語数に合わせて自動的に分割出力
- **柔軟な出力形式**:
    - 1ファイルへの結合出力（デフォルト）
    - HTMLとしてそのまま保存
...
| `-m, --max-size` | `1MB` | 1ファイルあたりの最大サイズ（例: `500KB`, `1.5MB`, `1048576`, 単語数指定は `50000w`） |
...
# 単語数ベースでの分割: 50,000単語ごとに分割
python scrip.py -u https://example.com/docs -o split_by_words.md -m 50000w
```
